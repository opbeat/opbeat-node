...
Instrumentation.prototype.start = function () {
  ...
  log.i.debug('shimming Module._load function')
  hook(MODULES, function (exports, name, basedir) {
    ...
    if (basedir) {
      ...
      try {
        ...
        log.i.debug('could not shim %s module: %s', name, e.message)
        return exports
      }
    } else {
    ...
    log.i.debug('shimming %s@%s module', name, version)
...
Instrumentation.prototype.addEndedTransaction = function (transaction) {
  if (this._agent.instrument) {
    log.i.debug({uuid: transaction._uuid}, 'adding transaction to queue')
    this._queue.add(transaction)
  } else {
    log.i.debug({uuid: transaction._uuid}, 'ignoring transaction')
  }
}
...
Instrumentation.prototype.endTransaction = function () {
  if (!this.currentTransaction) return log.i.debug('cannot end transaction - no active transaction found')
  this.currentTransaction.end()
}

Instrumentation.prototype.setDefaultTransactionName = function (name) {
  var trans = this.currentTransaction
  if (!trans) return log.i.debug('no active transaction found - cannot set default transaction name')
  trans.setDefaultName(name)
}

Instrumentation.prototype.setTransactionName = function (name) {
  var trans = this.currentTransaction
  if (!trans) return log.i.debug('no active transaction found - cannot set transaction name')
  trans.name = name
}

Instrumentation.prototype.buildTrace = function () {
  if (!this.currentTransaction) {
    log.i.debug('no active transaction found - cannot build new trace')
    return null
  }
