...
module.exports = function (express, agent, version) {
  ...
  if (!semver.satisfies(version, '^4.0.0')) {
    log.i.debug('express version %s not suppoted - aborting...', version)
    return express
  }

  log.i.debug('shimming express.Router.use function')
      ...
      if (typeof fn === 'string' && Array.isArray(this.stack)) {
        ...
        for (; offset < this.stack.length; offset++) {
          ...
          if (layer && fn !== '/' || (layer.regexp && !layer.regexp.fast_slash)) {
            log.i.debug('shimming layer.handle_request function (layer: %s)', layer.name)
            ...
            log.i.debug('skip shimming layer.handle_request (layer: %s, path: %s)', layer && layer.name || typeof layer, fn)
          }
        }
  ...
  log.i.debug('shimming express.static function')
  ...
  shimmer.wrap(express, 'static', function wrapStatic (orig) {
    ...
    Object.keys(orig).forEach(function (prop) {
      log.i.debug('copying property %s from express.static', prop)
