...
module.exports = function (ioredis, opbeat, version) {
  if (!semver.satisfies(version, '^2.0.0')) {
    log.i.debug('ioredis version %s not suppoted - aborting...', version)
    return ioredis
  }

  log.i.debug('shimming ioredis.Command.prototype.initPromise')
  shimmer.wrap(ioredis.Command && ioredis.Command.prototype, 'initPromise', wrapInitPromise)

  log.i.debug('shimming ioredis.prototype.sendCommand')
  ...
  function wrapSendCommand (original) {
    return function wrappedSendCommand (command) {
      ...
      log.i.debug({uuid: uuid, command: command && command.name}, 'intercepted call to ioredis.prototype.sendCommand')
