...
module.exports = function (redis, opbeat, version) {
  if (!semver.satisfies(version, '^2.0.0')) {
    log.i.debug('redis version %s not suppoted - aborting...', version)
    return redis
  }

  var proto = redis.RedisClient && redis.RedisClient.prototype
  if (semver.satisfies(version, '>2.5.3')) {
    log.i.debug('shimming redis.RedisClient.prototype.internal_send_command')
    shimmer.wrap(proto, 'internal_send_command', wrapInternalSendCommand)
  } else {
    log.i.debug('shimming redis.RedisClient.prototype.send_command')
  ...
  function wrapInternalSendCommand (original) {
    return function wrappedInternalSendCommand (commandObj) {
      ...
      log.i.debug({uuid: uuid, command: command}, 'intercepted call to RedisClient.prototype.internal_send_command')
