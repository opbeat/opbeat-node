...
module.exports = function (mysql, opbeat, version) {
  if (!semver.satisfies(version, '^2.0.0')) {
    log.i.debug('mysql version %s not suppoted - aborting...', version)
    return mysql
  }

  log.i.debug('shimming mysql.createConnection')
  shimmer.wrap(mysql, 'createConnection', wrapCreateConnection)

  log.i.debug('shimming mysql.createPool')
  shimmer.wrap(mysql, 'createPool', wrapCreatePool)

  log.i.debug('shimming mysql.createPoolCluster')
  ...
  function wrapCreatePool (original) {
    return function wrappedCreatePool () {
      ...
      log.i.debug('shimming mysql pool.getConnection')
...
function wrapQueryable (obj, objType, opbeat) {
  log.i.debug('shimming mysql %s.query', objType)
  ...
  function wrapQuery (original) {
    return function wrappedQuery (sql, values, cb) {
      ...
      log.i.debug({uuid: uuid}, 'intercepted call to mysql %s.query', objType)
      ...
      if (trace) {
        ...
        if (sqlStr) {
          log.i.debug({uuid: uuid, sql: sqlStr}, 'extracted sql from mysql query')
