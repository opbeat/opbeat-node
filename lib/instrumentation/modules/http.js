...
module.exports = function (http, agent) {
  log.i.debug('shimming http.Server.prototype.emit function')
  shimmer.wrap(http && http.Server && http.Server.prototype, 'emit', shared.instrumentRequest(agent, 'http'))

  log.i.debug('shimming http.request function')
  shimmer.wrap(http, 'request', shared.traceOutgoingRequest(agent, 'http'))

  log.i.debug('shimming http.ServerResponse.prototype.writeHead function')
      ...
      if (trans) {
        ...
        if (headers && typeof headers === 'object' && !Array.isArray(headers)) {
          Object.keys(headers).some(function (key) {
            ...
            log.i.debug({uuid: trans._uuid}, 'detected SSE response - ending transaction')
