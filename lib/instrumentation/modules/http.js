  ...
  function wrapWriteHead (original) {
    return function wrappedWriteHead () {
      ...
      if (trans) {
        ...
        // End transacton early in case of SSE or WebSocket
        if (headers && typeof headers === 'object' && !Array.isArray(headers)) {
          Object.keys(headers).some(function (key) {
            ...
            if (lkey === 'content-type' &&
                String(headers[key]).toLowerCase().indexOf('text/event-stream') === 0) {
              type = 'SSE'
            } else if (lkey === 'upgrade' &&
            ...
            debug('detected %s response - ending transaction %o', type, { uuid: trans._uuid })
