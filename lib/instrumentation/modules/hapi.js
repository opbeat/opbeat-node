...
module.exports = function (hapi, agent, version) {
  ...
  if (!semver.satisfies(version, '>=9.0.0')) {
    log.i.debug('hapi version %s not suppoted - aborting...', version)
    return hapi
  }

  log.i.debug('shimming hapi.Server.prototype.initialize')
    ...
    return function () {
      if (typeof this.ext === 'function') {
        this.ext('onPreAuth', function (request, reply) {
          log.i.debug('received Hapi onPreAuth event')
        ...
        this.ext('onPreResponse', function (request, reply) {
          log.i.debug('received Hapi onPreResponse event')
        ...
        log.i.debug('unable to enable automatic Hapi transaction naming')
      }
      ...
      if (typeof this.on === 'function') {
        this.on('request-error', function (req, err) {
          if (req.raw && req.raw.req) {
            ...
            log.i.debug('could not extract original request from hapi request')
            agent.captureError(err)
          }
        })
      } else {
        log.i.debug('unable to enable Hapi error tracking')
      }
