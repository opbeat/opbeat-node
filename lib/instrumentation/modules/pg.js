...
module.exports = function (pg, opbeat, version) {
  if (!semver.satisfies(version, '>=4.0.0 <7.0.0')) {
    log.i.debug('pg version %s not suppoted - aborting...', version)
    return pg
  }
...
function patchClient (Client, klass, opbeat) {
  log.i.debug('shimming %s.prototype.query', klass)
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (sql) {
      ...
      log.i.debug({uuid: uuid, sql: sql}, 'intercepted call to %s.prototype.%s', klass, name)
      ...
      if (trace) {
        ...
        if (typeof sql === 'string') {
          ...
          log.i.debug('unable to parse sql form pg module (type: %s)', typeof sql)
      ...
      function end () {
        log.i.debug({uuid: uuid}, 'intercepted end of %s.prototype.%s', klass, name)
        trace.end()
