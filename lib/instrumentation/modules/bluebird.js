...
module.exports = function (bluebird, opbeat, version) {
  ...
  if (!semver.satisfies(version, '>=2 <4')) {
    log.i.debug('bluebird version %s not suppoted - aborting...', version)
    return bluebird
  }

  log.i.debug('shimming bluebird.prototype functions: %s', BLUEBIRD_FNS)
  ...
  log.i.debug('shimming bluebird.config')
  wrap(bluebird, 'config', function wrapConfig (original) {
    return function wrappedConfig () {
      var result = original.apply(this, arguments)

      log.i.debug('shimming bluebird.prototype._attachCancellationCallback')
  ...
  if (semver.satisfies(version, '<3')) {
    log.i.debug('shimming bluebird.each')
    ...
    log.i.debug('shimming bluebird.prototype.each')
