...
Agent.prototype._config = function (opts) {
  ...
  // TODO: Scheduled to be removed in v5
  Object.defineProperty(this, 'logLevel', {
    get: function () {
      log.i.warn('The logLevel property is deprecated and will be removed in version 5')
      return log.i.level
    },
    set: function (level) {
      log.i.warn('The logLevel property is deprecated and will be removed in version 5')
...
Agent.prototype.start = function (opts) {
  ...
  this.on('error', function (err, uuid) {
    log.i.error({err: err, uuid: uuid}, 'Could not notify Opbeat')
  })
...
Agent.prototype.captureError = function (err, payload, cb) {
  ...
  if (!isMessage) {
    log.i.info({uuid: errUUID}, 'logging error with Opbeat')
    console.error(err.stack) // TODO: Remove in v5
  }
...
Agent.prototype.handleUncaughtExceptions = function (cb) {
  ...
  this._uncaughtExceptionListener = function (err) {
    ...
    agent.captureError(err, payload, function (opbeatErr, url) {
      if (opbeatErr) {
        log.i.error({err: opbeatErr, uuid: payload.extra.uuid}, 'Could not notify Opbeat')
      } else {
